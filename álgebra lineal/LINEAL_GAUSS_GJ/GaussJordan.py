"""Programa 2 que convierte la matriz dada en un archivo txt a una matriz escalonada """
...
def gaussjordan(**input1**):
    ...
    while (k<tam):
        while comprobarPivoteG(tam,matriz) == 1:
            while comprobarPivoteGJ(tam,matriz) == 1:
                while(i<ecu):
                    j=i+1
                    while(j<ecu):
        ...
        while(i<ecu):
            j=i+1
            while(j<ecu):
                IntercambioRenglon(tam,matriz,i,j)
